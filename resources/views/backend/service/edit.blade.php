            ...
            <form class="user" action="{{ route('backend.blog.aksi_edit', $blogs->id) }}" enctype="multipart/form-data" method="post">
                ...
                <div class="form-group">
                    <input type="text" name="title" class="form-control form-control-user" aria-describedby="emailHelp"
                        placeholder="Masukan judul" value="{{$blogs->title}}">
                </div>
                <div class="form-group">
                    <textarea name="description" class="form-control editor" placeholder="masukan deskripsi" cols="30" rows="3">{{$services->description}}</textarea>
                </div>
                <div class="form-group">
